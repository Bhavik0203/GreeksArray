            ...
            <nav className="-mb-px flex gap-4" aria-label="Tabs">
              {categories.slice(startIndex, startIndex + itemsToShow).map((category, index) => (
                <a
                  ...
                  className={`shrink-0 border-b-2 px-4 pb-4 text-sm font-medium 
                    ${activeCategory === category ? 'border-teal-500 text-teal-600' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700'}`}
                >
                  {category}
                </a>
              ))}
